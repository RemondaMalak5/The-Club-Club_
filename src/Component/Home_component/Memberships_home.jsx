  ...
  >
    {data.slice(0, 3).map((e, idx) => (
      <SwiperSlide key={e?.id || idx}>
        ...
        >
          {idx === 1 && (
            <div className="absolute inset-0 rounded-3xl bg-gradient-to-r from-[#08AC85]/10 to-[#00786F]/10 blur-2xl -z-10" />
          )}
          ...
          <div>
            ...
            <ul className="my-8 space-y-4">
              {e.features?.slice(0, 4).map((feat, i) => (
                <li
                  key={i}
      ...
      <div className=" hidden md:grid grid-cols-1 md:grid-cols-3 gap-12 items-stretch">
        {data.slice(0, 3).map((e, idx) => (
          <div
            key={idx}
            className={`
