...
const genderOptions = [
  ...
  const statusText = data?.myStatus?.statusText || "";
  ...
  useEffect(() => {
    if (
      location.state?.openBooking === true &&
      data &&
  ...
  }, [location.state?.openBooking, data]);
  ...
  const handleDependantChange = (e) => {
    ...
    const dependant = data?.myStatus?.dependants?.find(
      (item) =>
        String(item.addressId) === String(addressId)
    );
    ...
    const dependantStatus = dependant?.statusText || "";

    const normalizedDependantStatus = String(
      dependantStatus
    ).toLowerCase();
  ...
  const SubmitApplication = async (e) => {
    ...
    const body = {
      fullName:
        prefillData?.prefill?.fullName || "",

      programId,

      dateOfBirth:
        prefillData?.prefill?.dateOfBirth || "",
      ...
      nationality:
        prefillData?.prefill?.nationality || "",

      nationalId:
        prefillData?.prefill?.nationalId || "",
